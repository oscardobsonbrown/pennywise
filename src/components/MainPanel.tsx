...
const menuTriggerClassName =
    "p-1.5 rounded-lg text-(--color-text-muted) hover:text-(--color-text) hover:bg-(--color-bg-muted)";
        ...
        <div className="flex-1 flex flex-col h-screen overflow-hidden bg-(--color-bg)">
            ...
            <header className="h-12 px-3 sm:pl-6 sm:pr-3 flex items-center justify-between shrink-0 border-b border-(--color-border)">
                <div className="flex items-center gap-6 min-w-0 flex-1">
                    ...
                    <Menu.Root>
                        <Menu.Trigger
                            className={`${menuTriggerClassName} -ml-1.5`}
                        >
                            <svg
                                width="16"
                                height="16"
                        ...
                        <Menu.Portal>
                            <Menu.Positioner className="z-50" sideOffset={8}>
                                <Menu.Popup
                                    className={`${menuPopupClassName} min-w-[180px]`}
                                >
                                    <Menu.Item
                    ...
                    >
                        {visibleItems.map((item) => (
                            <button
                                ...
                                className={`px-2.5 py-1 text-sm font-medium rounded-lg shrink-0 ${
                        ...
                        <button
                            onClick={props.onOpenStart}
                            className="w-7 h-7 flex items-center justify-center rounded-lg text-(--color-text-muted) hover:text-(--color-text) hover:bg-(--color-bg-muted) shrink-0"
                        ...
                        {hasOverflow && (
                            <Menu.Root>
                                <Menu.Trigger
                                    className={`${menuTriggerClassName} px-2.5 py-1 text-sm font-medium`}
                                >
                                    ···
                                </Menu.Trigger>
                                <Menu.Portal>
                                    ...
                                    >
                                        <Menu.Popup
                                            className={`${menuPopupClassName} min-w-[100px]`}
                                        >
            ...
            {props.view === "loading" ? (
                ...
                <div className="flex-1 overflow-y-auto">
                    <ReceiptView
                        data={props.data}
                        onOpenChat={
                            !props.isChatOpen ? props.onToggleChat : undefined
                        }
                    />
                </div>
            ) : null}
